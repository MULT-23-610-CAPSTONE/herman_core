...
if runtime_OS == "Linux":
    _kromek = cdll.LoadLibrary('/usr/lib/libSpectrometerDriver.so')
...
def Kromek_Acquisition_Start(detectorID, liveTime_sec=0, realTime_sec=0):
    ...
    # c_realTime = int(realTime_sec * 1000)
    # c_liveTime = int(liveTime_sec * 1000)
    if int(liveTime_sec) > 0:
        ConsolePrint("Begin acquisition (Live time preset): " + str(c_liveTime) + " ms", isVerboseOutput=True)
    else:
        ConsolePrint("Begin acquisition (Real time preset): " + str(c_realTime) + " ms", isVerboseOutput=True)
    ...
    # success = _kromek.kr_BeginDataAcquisition(c_desiredDetectorID)
...
def ExampleCallback_DataReceived(event):
    ...
    # print(("Channel: " + str(event.channel) + "     , Counts: " + str(event.counts)))
    # my_dict = {"Channel":[], "Count":[]}
    # my_dict["Channel"].append(event.channel)
    # my_dict["Count"].append(event.counts)
    # print(my_dict)
    new_list.append(event.channel)
...
if __name__ == '__main__':
    ...
    while nextDetectorID:
        ...
        try:
            ...
            # to find every unique value in the list
            num=0
            ...
            i=0
            for x in list_set:
                dictionary[i] = (x, new_list.count(x))
                i+=1
            
            j = 0
            sort_orders = sorted(dictionary.items(), key=lambda x: x[1][0], reverse=0)
            for i in sort_orders:
                # print("Channel, Count: ", i[1])
                s.write("Channel:  " + str(sort_orders[j][1][0]) + "\tCounts:  " + str(sort_orders[j][1][1]) + "\n")
                j+=1
            ...
            #     s.write(" Channel: " + str(sort_orders[j][1][0]) + " Counts: " + str(sort_orders[j][1][1]) + "\n")
            #     j+=1
            # print(sort_orders[10][1][0]) # channel
            # print(sort_orders[10][1][1]) # count
